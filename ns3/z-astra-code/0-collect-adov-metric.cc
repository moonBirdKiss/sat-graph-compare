...
int main(int argc, char *argv[])
{
    ...
    Simulator::Schedule(Seconds(1.0), &Ipv4GlobalRoutingHelper::PopulateRoutingTables);
    // 模拟每隔1分钟，重新生成路由表
    for (int i = 0; i < SimluationTime; i += 60)
    {
        Simulator::Schedule(Seconds(i), &Ipv4GlobalRoutingHelper::RecomputeRoutingTables);
    }
