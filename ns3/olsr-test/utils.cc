                ...
                for (int i = 0; i < size; ++i)
                {
                    for (int j = i + 1; j < size; ++j)
                    {
                        ...
                        if (res)
                        {
                            if (flag)
                            {
                                satGraph->RecoverLink(i, j, Simulator::Now());
                            }
                            else
                            {
                                satGraph->TearDownLink(i, j, Simulator::Now());
                            }
                        }
                    }
                }
...
{
    for (float time = 1.; time < simluationTime; time = time + 10)
    {
...
void changeSats(SatGraph *satGraph, int timeInterval, int startTime, int endTime)
{
    ...
    // for single test
    satGraph->TearDownLink(0, 2, Seconds(5.0));
}
